...
const BookById = (props) => {
    ...
    if (loading) return <h2>Loading...<Smiley desc={'load'} smiley={'👨‍💻'} /></h2>;
    if (error) return <p>Something went wrong! <Smiley desc={'sad'} smiley={'😡'} /></p>;
    const { title, imageUrl, duration, author, description } = data.book
    return (
        <>
            ...
            <BookDivComponent>
                ...
                <BookInfoContainer>
                    ...
                    <BookDuration><BookSpan>Duration: </BookSpan> {duration} Hours (Audio Book)</BookDuration>
